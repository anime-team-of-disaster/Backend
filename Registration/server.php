...
if (isset($_POST['reg_user'])) {
  ...
  if ($password_1 != $password_2) {
	array_push($errors, "Hasła nie pokrywają się");
  }
  ...
  if ($user) { // if user exists
    ...
    if ($user['email'] === $email) {
      array_push($errors, "Email jest w bazie danych");
    }
  }
  ...
  if (count($errors) == 0) {
  	...
  	$_SESSION['success'] = "Zostałeś zalogowny";
    ...
    if (count($errors) == 0) {
        ...
        if (mysqli_num_rows($results) == 1) {
          ...
          header('location: index.php');
            ...
            array_push($errors, "Złe hasło/login");
        }
    }
  }
